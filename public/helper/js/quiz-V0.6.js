var app = new Vue({
    ...
    computed: {
        ...
        renderCurrentQuestionTitle: function(){
            const question = this.getCurrentQuestion();
            if(question){
                let title = question.question_title;
                if(this.language != 'en'){
                    const title_localized = question['question_title_'+this.language];
                    title = title_localized ? title_localized: title;
                }
                return title;
            }
            return '';
    ...
    methods: {
        ...
        initTopic: async function(){
            ...
            await this.loadTopic('chapter').then(async function(){
                    ...
                    await app.loadTopic('exam').then(async function(){
                        await app.loadTopic('process').then(async function(){
        ...
        renderChoiceFeedback: function(choice){
            let feedback = choice.feedback;
            if(this.language != 'en'){
                const answer_localized = choice.transcodes['feedback_'+this.language];
                feedback = answer_localized ? answer_localized: answer;
            }
            return feedback;
        },
        ...
        markExam:async function(confirmation){
            ...
            if(app.overallScore >= 75){
                this.ScoreMsg = '<h4 style="font-weight:bold">:'+quizAttr.translation.overallPerformance+'<b style="color:green;" class="mx-2">'+quizAttr.translation.passed+'</b></h4>You have passed the exam, congratulations.';
            }else{
        ...
        topicURL: function(topic_key, topic_id, view, part_id = null){
            var url = quizAttr.base_url+'/PremiumQuiz/'+this.package_id+'/'+topic_key+'/'+topic_id+'/preview/'+view;
            if(part_id){
                url += '?part_id='+part_id;
            }
            return url;
