                    ...
                    <form class="pt-15 pb-10" novalidate="novalidate" id="kt_create_account_form"  >
                                    ...
                                    <div class="container">
                                        ...
                                        <div class="row gy-5 mt-5 text-center">
                                            ...
                                            <div class="col-md-3 p-2" v-for="i in part_data" :key="i.id">
                                                    ...
                                                    <label class="btn btn-outline btn-active-light-primary" :class="{'selected': {{$events-> part_id}} == i.id}" style="width:100%; box-shadow: 0px 0px 5px 0px #aaa !important; border-radius:10px;" :for="'part_' + i.id">
                                                      <!--begin::Info-->
                                                      <div style="width: 80px; height: 80px;" :class="{'border-black': {{$events-> part_id}} != i.id, 'border-white': {{$events-> part_id}} == i.id}" class="rounded-circle  mx-auto d-flex align-items-center justify-content-center">
                                                        <span>@{{ i.id }} </span>
                                                      </div>
                                                      <div>
                                                        <h4 class="h2 mt-3" style="font-size: 15px;" :style="{ color: {{$events-> part_id}} == i.id ? 'white' : 'black' }">@{{i.title}}</h4>
                                                      </div>
                                                      <!--end::Info-->
                                                    </label>
                                    ...
                                    <div class="container">
                                        ...
                                        <div class="row gy-5 mt-5 text-center">
                                            <div class="col-md-3 p-2" v-for="i in chapter_data" :key="i.id">
                                                    ...
                                                    <label class="btn btn-outline btn-active-light-primary" :class="{'selected': {{$events-> chapter_id}} == i.id}" style="width:100%; box-shadow: 0px 0px 5px 0px #aaa !important; border-radius:10px;" :for="'chapter_' + i.id">
                                                      <!--begin::Info-->
                                                      <div style="width: 80px; height: 80px;" :class="{'border-black': {{$events-> chapter_id}} != i.id, 'border-white': {{$events-> chapter_id}} == i.id}" class="rounded-circle  mx-auto d-flex align-items-center justify-content-center">
                                                        <span>@{{ i.id }} </span>
                                                      </div>
                                                      <div>
                                                        <h4 class="h2 mt-3" style="font-size: 15px;" :style="{ color: {{$events-> chapter_id}} == i.id ? 'white' : 'black' }">@{{ i.title }}</h4>
                                                      </div>
                                                      <!--end::Info-->
                                                    </label>
                        ...
                        <div data-kt-stepper-element="content">
                            ...
                            <div class="w-100">
                                ...
                                <div class="container">
                                    <div class="text-center">
                                        <h5 class="h1">اضافة تفاصيل دورة البث المباشر</h5>
                                        <p class="m-0 fs-3 text-gray-500">قم  باضافة تفاصيل دورة البث المباشر</p>
                                    </div>
                                    <div class="my-3">
                                        <h5>اسم دورة البث المباشر</h5>
                                        <input class="form-control" type="text" placeholder="اسم دورة البث المباشر" name="title" id="title"  v-model="title" />
                                    </div>

                                    <div class="my-3">
                                        <h5>إجمالي وقت محاضرات دورة البث المباشر</h5>
                                        <input class="form-control" type="number" placeholder="إجمالي وقت محاضرات دورة البث المباشر" name="total_time" id="total_time"  v-model="total_time" />
                                    </div>
                                    <div class="my-3">
                                        <h5>عدد محاضرات دورة البث المباشر الإجمالي</h5>
                                        <input class="form-control" type="number" name="lectures" id="lectures"  v-model="lectures" placeholder="عدد محاضرات دورة البث المباشر الإجمالي"/>
                                    </div>
                                    <div class="my-4">
                                        <div class="row gy-5">
                                    ...
                                    <div class="my-3">
                                        <h5>تحميل صورة دورة البث المباشر</h5>
                                        <div class="global-input position-relative " style="height: 50px;border: 1px solid #eee">
                                            ...
                                            <div style =" margin-top:10px; "><a  class="rounded w-150px " target="_blank" href="{{asset('events/imgs/' . $events->img)}}">{{$events->img}}</a></div>
                                        </div>
                                    </div>
                                    <div style =" margin-top:40px;">
                                        <div class="my-4">
                                            <h4 class="m-0 my-4">وصف دورة البث المباشر</h4>
                                    ...
                                    <div class="my-4">
                                        ...
                                        <input type="text" class="form-control" name="google_meet" id="google_meet"  v-model="google_meet"  placeholder="رابط google Meet"/>
                                    </div>
                                    <div class="my-4">
                                        <h4>رابط zoom</h4>
                                        <input type="text" class="form-control" name="zoom" id="zoom"  v-model="zoom"  placeholder="رابط zoom"/>
                                    </div>
                                    <div class="my-4">
                                        <h4>الشهادة</h4>
                                    ...
                                    <div class="my-4">
                                        ...
                                        <input type="hidden" name="eventDays" v-model="JSON.stringify(eventDays)">

                                        <!-- Display the added event days -->
                                        <div id="days-container" style="margin-top:20px;">
                                            {{-- <div class="day-item" v-for="(day, index) in eventDays" :key="index">
                                                <span>@{{ day.date }}</span>
                                                <span>@{{ day.from }}</span>
                                                <span>@{{ day.to }}</span>
                                            ...
                                            <table class="table" style="width:50%; border:1px solid #e4e6ef; border-radius: 10px !important; ">
                                                ...
                                                <tbody>
                                                    ...
                                                    <!-- Display previous event days -->
<tr v-for="(day, index) in previousEventDays" >
    <td>@{{ day.day }}</td>
    <td>@{{ day.from }}</td>
    <td>@{{ day.to }}</td>
  ...
  <!-- Display new event days -->
  <tr v-for="(day, index) in eventDays" >
    <td>@{{ day.day }}</td>
    <td>@{{ day.from }}</td>
    <td>@{{ day.to }}</td>
      ...
      <i class="fa-solid fa-trash fs-1"></i>
                                                ...
                                                <div class="d-flex align-items-center p-5">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p class="m-0 mx-3 text-gray-500" style="font-size: 14px;">
                                                            ...
                                                            <span class="mx-3">{{$events->created_at}}</span>
                                                        </p>
                                                    </div>
                                                </div>
                                                <p class="m-0 px-5">
                                                    <a href="{{route('event.delete',$events->id)}}"> <i role="button" class="fa-solid fa-trash-can fs-3 want_delete_btn"></i></a>
                                                </p>
                                            </div>
                                        </div>
                        ...
                        <div class="d-flex flex-stack pt-10">
                            ...
                            <div>
                                ...
                                <a type="button" class="regular-black-btn btn mx-2" href="{{route('event.show')}}" >حفظ كمسودة </a>
...
<script>
    ...
    var app = new Vue({
        ...
        data: {
            ...
            google_meet:'{{$events->google_meet}}',
            zoom:'{{$events->zoom}}',
            certification:'{{$events->certification}}',
            certification_title:'{{$events->certification_title}}',
            price:'{{$events->price}}',
            discount:'{{$events->discount}}',
            price_after_discount:'{{$events->price_after_discount}}',
            dayInput: '',
        fromInput: '',
        toInput: '',
        eventDays: [],
        previousEventDays: [
                        @foreach(\App\Models\EventDay::where('event_id', $events->id)->get() as $day)
                    {
                        'day': '{{$day->day}}',
                        'from': '{{$day->from}}',
                        'to': '{{$day->to}}',
  ...
  this.what_you_learn = '{{$events->what_you_learn}}';
  this.requirments = '{{$events->requirments}}';
  this.who_package_for = '{{$events->who_package_for}}';
  this.enroll_message = '{{$events->enroll_message}}';
  this.getParts();
  this.getChapters();
  this.getLessons();
    ...
    formData.append('google_meet', this.google_meet);
    formData.append('zoom', this.zoom);
    ...
    formData.append('eventDays', JSON.stringify(this.eventDays));
    ...
    $.ajax({
      ...
      contentType: false,
      data: formData,
      url: '{{ route('event.update', $events->id) }}',
      success: function(response) {
                  ...
                  new swal({
                    ...
                    window.location.href = "{{ route('event.show') }}";
                  });
                },
  ...
  // Add the day object to the eventDays array
  this.eventDays.push(dayObj);
...
removeDayFromPrevious: function(index) {
  // Remove the day at the specified index from the previousEventDays array
  this.previousEventDays.splice(index, 1);
},

removeDayFromNew: function(index) {
  // Remove the day at the specified index from the newEventDays array
